...
module Aws
  module CloudFront
        describe UrlSigner do
            ...
            let(:key_pair_id) { "CF_KEYPAIR_ID" }
            let(:private_key_path) { "#{File.dirname(__FILE__)}/cf_private_key.pem" }
            let(:signer) { UrlSigner.new(key_pair_id, private_key_path) }
        	let(:expires) { 1357034400 } # January 1, 2013 10:00 am UTC (Unix timestamp)

            describe '#initialize' do
                it 'takes keypair id and privatekey path' do
                    expect(signer.class).to eq(UrlSigner)
                end
            end
            ...
            describe '#signed_url' do
                it 'raises error if url is invaild' do
                    expect {
                        signer.signed_url("http:://what_ever://ilegal/url")
