...
module Aws::Greengrass
  describe EndpointProvider do
    ...
    context 'For region dataplane-us-gov-east-1 with FIPS disabled and DualStack disabled' do
      let(:expected) do
        {"endpoint"=>{"properties"=>{"authSchemes"=>[{"signingName"=>"greengrass", "signingRegion"=>"us-gov-east-1", "name"=>"sigv4"}]}, "url"=>"https://greengrass-ats.iot.us-gov-east-1.amazonaws.com"}}
      end

      it 'produces the expected output from the EndpointProvider' do
        params = EndpointParameters.new(**{:use_dual_stack=>false, :use_fips=>false, :region=>"dataplane-us-gov-east-1"})
        endpoint = subject.resolve_endpoint(params)
        expect(endpoint.url).to eq(expected['endpoint']['url'])
        expect(endpoint.headers).to eq(expected['endpoint']['headers'] || {})
        expect(endpoint.properties).to eq(expected['endpoint']['properties'] || {})
    ...
    context 'For region dataplane-us-gov-west-1 with FIPS disabled and DualStack disabled' do
      let(:expected) do
        {"endpoint"=>{"properties"=>{"authSchemes"=>[{"signingName"=>"greengrass", "signingRegion"=>"us-gov-west-1", "name"=>"sigv4"}]}, "url"=>"https://greengrass-ats.iot.us-gov-west-1.amazonaws.com"}}
      end

      it 'produces the expected output from the EndpointProvider' do
        params = EndpointParameters.new(**{:use_dual_stack=>false, :use_fips=>false, :region=>"dataplane-us-gov-west-1"})
        endpoint = subject.resolve_endpoint(params)
        expect(endpoint.url).to eq(expected['endpoint']['url'])
        expect(endpoint.headers).to eq(expected['endpoint']['headers'] || {})
        expect(endpoint.properties).to eq(expected['endpoint']['properties'] || {})
