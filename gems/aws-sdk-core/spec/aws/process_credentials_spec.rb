...
module Aws
  describe ProcessCredentials do
    ...
    it 'will read credentials from a process' do
      creds = ProcessCredentials.new('echo \'{"Version":1,"AccessKeyId":"AK_PROC1","SecretAccessKey":"SECRET_AK_PROC1","SessionToken":"TOKEN_PROC1","CredentialScope":"CREDENTIAL_SCOPE_PROC1"}\'').credentials
      ...
      expect(creds.credential_scope).to eq('CREDENTIAL_SCOPE_PROC1')
    end

    it 'will throw an error when invalid JSON is returned' do
      expect {
        ProcessCredentials.new('echo \'{"Version":1,"AccessKeyId":"","SecretAccessKey":""\'').credentials
    ...
    it 'will throw an error when the process credentials payload version is invalid' do
      expect {
        ProcessCredentials.new('echo \'{"Version":3,"AccessKeyId":"","SecretAccessKey":""}\'').credentials
      }.to raise_error(Errors::InvalidProcessCredentialsPayload)
    end

    it 'will throw an error when the process credentials payload is malformed' do
      expect {
        ProcessCredentials.new('echo \'{"Version":1}\'').credentials
      }.to raise_error(Errors::InvalidProcessCredentialsPayload)
    end

    it 'will throw an error and expose the stderr output when the credential process has a nonzero exit status' do
      expect {
        ProcessCredentials.new('>&2 echo "Credential Provider Error"; false').credentials
      }.to raise_error(Errors::InvalidProcessCredentialsPayload)
