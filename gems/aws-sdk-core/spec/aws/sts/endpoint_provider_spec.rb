...
module Aws::STS
  describe EndpointProvider do
    ...
    context 'UseGlobalEndpoint with legacy region `ap-northeast-1`' do
      let(:expected) do
        {"endpoint"=>{"properties"=>{"authSchemes"=>[{"signingName"=>"sts", "signingRegion"=>"us-east-1", "name"=>"sigv4"}]}, "url"=>"https://sts.amazonaws.com"}}
      end
      ...
      it 'produces the correct output from the client when calling get_caller_identity' do
        ...
        expect_auth({"signingName"=>"sts", "signingRegion"=>"us-east-1", "name"=>"sigv4"})
    ...
    context 'UseGlobalEndpoint with Non-legacy region `us-east-3`' do
      let(:expected) do
        {"endpoint"=>{"properties"=>{"authSchemes"=>[{"signingName"=>"sts", "signingRegion"=>"us-east-3", "name"=>"sigv4"}]}, "url"=>"https://sts.us-east-3.amazonaws.com"}}
      end
      ...
      it 'produces the correct output from the client when calling get_caller_identity' do
        ...
        expect_auth({"signingName"=>"sts", "signingRegion"=>"us-east-3", "name"=>"sigv4"})
        resp = client.get_caller_identity(
        )
        expected_uri = URI.parse(expected['endpoint']['url'])
        expect(resp.context.http_request.endpoint.to_s).to include(expected_uri.host)
    ...
    context 'UseGlobalEndpoint with unset region and custom endpoint' do
      ...
      it 'produces the expected output from the EndpointProvider' do
        params = EndpointParameters.new(**{:endpoint=>"https://example.com", :use_fips=>false, :use_dual_stack=>false, :use_global_endpoint=>false})
        endpoint = subject.resolve_endpoint(params)
        expect(endpoint.url).to eq(expected['endpoint']['url'])
        expect(endpoint.headers).to eq(expected['endpoint']['headers'] || {})
        expect(endpoint.properties).to eq(expected['endpoint']['properties'] || {})
