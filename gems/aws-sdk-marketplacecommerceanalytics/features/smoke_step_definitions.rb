...
Given(/I create a client in region '(.*?)'/) do |region|
  @regional_client = Aws::MarketplaceCommerceAnalytics::Client.new(region: region)
end

Given(/I create a client with endpoint '(.*?)'/) do |endpoint|
  @regional_client = Aws::MarketplaceCommerceAnalytics::Client.new(endpoint: endpoint)
end
