...
describe 'Client Interface:' do
  describe 'Transfer Encoding' do
    describe 'authtype: v4-unsigned-body' do
      before(:all) do
        SpecHelper.generate_service(['LegacyAuthTransferEncoding'], multiple_files: false)
      end

      let(:client) do
        LegacyAuthTransferEncoding::Client.new(stub_responses: true)
      end
    ...
    describe 'unsignedPayload' do
      ...
      it 'adds `Transfer-Encoding` header for `unsignedPayload` and v4 auth' do
        resp = client.unsign_streaming(body: StringIO.new('hey'))
        expect(resp.context.http_request.headers['Transfer-Encoding']).to be(nil)
        expect(resp.context.http_request.headers['Content-Length']).to eq('3')
