      ...
      class Logging < Plugin
        describe Handler do
          ...
          it 'populates the request context with timing information' do
            resp = handle!
            resp.context[:started_at].must_be_kind_of(Time)
            resp.context[:completed_at].must_be_kind_of(Time)
            assert(resp.context[:started_at] < resp.context[:completed_at])
          end

          it 'sends the formatted message to the logger' do
            handle!
            @log_dev.messages.must_match(/test-formatter-log-msg/)
          end
          ...
          it 'sends the message at the right log level' do
            ...
            handle!
            @log_dev.messages.must_equal(nil)
          end
